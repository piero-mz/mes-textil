...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    // Merma total
    const totalMerma = (mermaData || []).reduce((acc, m) => acc + (m.cantidad_kg || 0), 0)

    // Eficiencia: promedio de avance de lotes
    ...
    // Alertas dinámicas desde control de calidad
    ...
    ;(controles || []).forEach((c: any) => {
      if (c.resultado === 'RECHAZADO') {
        ...
        alertasGeneradas.push({ msg: `Lote ${c.lote?.codigo} requiere revisión — observado en control`, type: 'warning' })
      } else if (c.resultado === 'APROBADO') {
        alertasGeneradas.push({ msg: `Lote ${c.lote?.codigo} aprobado en control de calidad`, type: 'success' })
      }
    })
    ;(mermaData || []).forEach((m: any) => {
      if (m.porcentaje > 5) {
        alertasGeneradas.push({ msg: `Merma elevada registrada: ${m.porcentaje}% — supera límite del 5%`, type: 'warning' })
      }
    })
              ...
              <div className="flex items-end gap-5 h-36">
                {barData.map(b => (
                  <div key={b.label} className="flex flex-col items-center gap-2 flex-1">
                    <span className="text-[#2696F2] text-xs font-semibold">
                      {b.val > 0 ? `${b.val}%` : '—'}
                    </span>
                    <div className="w-full rounded-t-md bg-[#1E2D42] relative" style={{ height: '120px' }}>
                      <div
                        className="absolute bottom-0 w-full bg-[#2696F2] rounded-t-md transition-all duration-700"
